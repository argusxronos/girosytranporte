<?php
	...
	if($TDOC == 'FACTURA')
	{
		if ($TPER == 'REMITENTE')
		{
			$db_giro->query("SELECT `e_persona`.`id_persona`
						, `e_persona`.`per_ruc`
						FROM `e_persona`
						WHERE `e_persona`.`id_persona` = '".$idClient."'
			...
			{
				$id_persona = $res[0][0];
				echo "formObj.txt_remit_dni.value = '".utf8_encode($res[0][1])."';\n";
			}else{
				echo "formObj.txt_remit_dni.value = '';\n";
			}
			if ($id_persona > 0)
			{
		...
		{
			$db_giro->query("SELECT `e_persona`.`per_num_dni`
			...
			if(count($res)>0)
			{
				echo "formObj.txt_consig_dni.value = '".utf8_encode($res[0][0])."';\n";
				echo "formObj.txt_consig_direccion.value = '".utf8_encode($res[0][1])."';\n";
			}else
			{
				echo "formObj.txt_consig_dni.value = '';\n";
			}
		}
	}
	elseif($TDOC == 'BOLETA')
	{
		if ($TPER == 'REMITENTE')
		{
			...
			if(count($res)>0)
			{
				echo "formObj.txt_remit_dni.value = '".utf8_encode($res[0][0])."';\n";
			}else{
				echo "formObj.txt_remit_dni.value = '';\n";
			}
		}
	...
	elseif($TDOC == 'GUIA REMISION')
	{
		if ($TPER == 'REMITENTE')
		{
			...
			if ($id_persona > 0)
			{
				...
				{
					echo "formObj.txt_remit_direccion.value = '".utf8_encode('AGENCIA')."';\n";
				}
			}else
			{
				echo "formObj.txt_remit_direccion.value = '".utf8_encode('INGRESE DIRECCIÓN')."';\n";
		...
		elseif ($TPER == 'CONSIGNATARIO')
		{
			...
			{
				$id_persona = $res[0][0];
				echo "formObj.txt_consig_dni.value = '".utf8_encode($res[0][1])."';\n";
			}else{
				echo "formObj.txt_consig_dni.value = '';\n";
			}
			if ($id_persona > 0)
			{
				$db_giro->query("SELECT 
								IF(LENGTH(IFNULL(`e_direccion`.`dir_text`,'')) = 0,'AGENCIA', `e_direccion`.`dir_text` )
								FROM `e_direccion`
								WHERE `e_direccion`.`id_persona` = '".$id_persona."'
								AND `e_direccion`.`id_oficina` = ".$IDOFICINA."
								LIMIT 1;");
				...
				if(count($res)>0)
				{
					...
					echo "formObj.txt_consig_direccion.focus();\n";
					echo "formObj.txt_consig_direccion.select();\n";
				}
				else
				{
					echo "formObj.txt_consig_direccion.value = '".utf8_encode('AGENCIA')."';\n";
				}
			}else
			{
				echo "formObj.txt_consig_direccion.value = '".utf8_encode('INGRESE DIRECCIÓN')."';\n";
			}
		}
	}
?>