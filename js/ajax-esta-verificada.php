<?php
	...
	$db_giro->query("UPDATE `g_entrega` SET `ent_verificada`=  IF(`ent_verificada` = 0, 1, 0) WHERE `id_movimiento` = '".$id."';");
	...
	$db_giro->query("SELECT COUNT(`g_operacion`.`id_movimiento`) AS `id_movimiento`
					...
					AND `g_operacion`.`ope_tipo_operacion` = 5;");
	...
	if ($existe_ope == 0)
	{
		...
		$db_giro->query("INSERT INTO `g_operacion`
						...
						VALUES
						(".$id.", 5,".$_SESSION['ID_OFICINA'].", ".$_SESSION['ID_USUARIO'].", CURDATE(), CURTIME(),'Verificación de GIRO PAGADO','".$pc_nom_ip."');");
	}
	else
	{
		...
		`ope_fecha` = CURDATE(),
		`ope_hora` = CURTIME(),
		`ope_detalle` = 'Verificación de GIRO PAGADO',
		`nom_pc_ip` = '".$pc_nom_ip."'
		WHERE `id_movimiento` = ".$id."
		AND `ope_tipo_operacion` = 5;");
	}
?>