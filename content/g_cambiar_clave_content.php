...
?>
	...
	<div class="column1-unit">
	  ...
	  <?php echo '<h3>'.date("l j \d\e F, Y, h:i A").'</h3>'; ?>
		<form method="get" action="g_cambiar_clave.php" name="buscar_giro" >
			...
			if (isset($_GET['btn_buscar']) && count($Giros_Array) > 0)
			{
				...
				for ($fila = 0; $fila < count($Giros_Array); $fila++)
				{
					...
					echo "<tr>";
							...
							if ($estado == 'NO')
							{
								echo '<td style="text-align:center;"><a href="g_cambiar_clave.php?ID='.$id.'" ><img src="./img/operacion/Symbol-Update.png" width="24" height="24" title="Modificar Giro." /></a></td>';
							}
					echo "</tr>";
				}
...
?>
	...
	<div class="column1-unit">
		...
		<form action="g_cambiar_clave_action.php?insert" method="post" name="change_form" target="_self">
			<table width="100%" border="0">
              <tr>
				<th>Documento :</th>
				<td colspan="3"><span>
                	<?php echo $Giro_Array[0][8] .' - ' .$Giro_Array[0][9]; ?></span>                </td>
			  </tr>
			  <tr>
			  ...
			  <tr>
				<th>Consignatario:</th>
				<td  colspan="3"><span><?php echo $Giro_Array[0][5]; ?></span></td>
			  </tr>
              <tr>
				<th>Nueva Clave:</th>
				<td  colspan="3"><input type="password" value="" name="txt_clave" class="field" title="Clave de Seguridad." tabindex="1"  style="width:150px; text-transform:uppercase; font-size:16px; font-weight:bold;" onfocus="this.select()" maxlength="4" onkeyup="extractNumber(this,2,false);" onkeypress = "return handleEnter(this, event);" />
				  <strong>Clave Anterior: </strong><?php echo $Giro_Array[0][7]; ?>              
              </tr>
			  <tr>
				<th colspan="4" style="height:5px;"></th>
			  </tr>
			  ...
			  <tr>
				<th colspan="4" scope="row" style="text-align:center;"><span>
				  <input name="btn_guardar" id="btn_guardar" type="submit" class="button" value="Guardar" tabindex="5" onclick="this.disabled = 'true'; this.value = 'Enviando...'; document.change_form.submit();" />
