...
<div class="main-content">
	...
	<!-- Pagetitle -->
	<h1 class="pagetitle">Autorizar Giro</h1>
    <?php 
	if (!isset($_GET['ID']))
	{
?>
	...
	<div class="column1-unit">
	  ...
	  <?php echo '<h3>'.date("l j \d\e F, Y, h:i A").'</h3>'; ?>
		<form method="get" action="g_autorizar.php?insert" name="buscar_giro" >
			<table width="100%" border="0">
                ...
                <tr>
					<th>Agencia<br />
				    Origen : </th>
					<th><select name="cmb_agencia_origen" class="combo" title="Agenia de origen del giro." tabindex="3" >
                      <?php
					...
					Destino : </th>
					<th>
						<select name="cmb_agencia_destino" class="combo" title="Agenia de origen del giro." tabindex="7" >
							<?php
								if (count($Oficina_Array) == 0)
								{
									echo '<option value="">[ NO HAY OFICINAS...! ]</option>';
			...
			if (count ($Giros_Array) > 0)
			{
				...
				for ($fila = 0; $fila < count($Giros_Array); $fila++)
				{
					...
					echo "<tr>";
						...
						if ($estado == 'NO')
						{
							echo '<td style="text-align:center;"><a href="g_autorizar.php?ID='.$id.'" ><img src="./img/operacion/Warning-Shield.png" width="24" height="24" title="Autorizar Giro." /></a></td>';
						}
					echo "</tr>";
				}
				if (!isset($_GET['btn_buscar']))
				{
					...
					echo '<tr>';
						$url = 'g_autorizar.php?';//curPageURL();
...
?>
	...
	<div class="column1-unit">
		...
		<form action="g_autorizar_action.php?insert" method="post" name="entrega_form" target="_self">
			<table width="100%" border="0">
			  <tr>
			  ...
			  <tr>
				<th>Forma Entrega:</th>
				<td colspan="3"><span><?php echo strtoupper(utf8_encode($Giro_Array[0][8])); ?></span></td>
			  </tr>
			  <tr>
				<th>Consignatario:</th>
				<td  colspan="3"><input type="hidden" id="txt_consig_hidden" name="txt_consig_ID" value="<?php echo $Giro_Array[0][4]; ?>" />
					<input type="text" value="<?php echo utf8_encode($Giro_Array[0][5]); ?>" name="txt_consig" class="field" style="width:455px;text-transform:uppercase;" title="Apellidos del Consignatario." tabindex="2" onkeypress="return acceptletras(this, event)" onkeyup="ajax_showOptions(this,'getPersonByLetters',event)" autocomplete="off" onfocus="this.select();" /></td>
			  </tr>
			  <tr>
				<th colspan="4" style="height:5px;"></th>
			  </tr>
			  <tr>
				...
				<td colspan="4"><input type="text" name="txt_observ" onkeypress="return handleEnter(this,event);" onfocus="this.select()" style="width:98%;text-transform:uppercase;" tabindex="3" /></td>
			  </tr>
			  <tr>
			  ...
			  <tr>
				<th colspan="4" scope="row" style="text-align:center;"><span>
				  <input name="btn_guardar" id="btn_guardar" type="submit" class="button" value="Guardar" tabindex="4" onclick="this.disabled = 'true'; this.value = 'Enviando...'; document.entrega_form.submit();" />
				  <input type="button" name="btn_regresar" id="btn_regresar" class="button" value="Regresar" tabindex="5" onclick="document.location.href='g_entrega.php';" />
				</span></th>
			  </tr>
			</table>
		</form>
