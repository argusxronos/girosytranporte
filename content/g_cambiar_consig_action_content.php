<?php
	...
	if (isset($_GET['insert']))
	{
		...
		{
			MsjErrores('No se puede MOdificar este Giro, intentelo de nuevo.');
		}
		else
		{
			...
			if ($Error == false)
			{
				...
				if ($ya_existe == 0)
				{
					...
					$db_giro->query("UPDATE `g_movimiento` 
									SET `id_consignatario`= ".$id_consig."
									, `esta_copiado` = 0
					...
					else
					{
						...
						$db_giro->query("SELECT COUNT(`g_operacion`.`id_movimiento`) AS `id_movimiento`
										...
										AND `g_operacion`.`ope_tipo_operacion` = 7;");
						...
						if ($existe_ope == 0)
						{
							...
							$db_giro->query("INSERT INTO `g_operacion`
											...
											VALUES
											(".$id_movimiento.", 7,".$_SESSION['ID_OFICINA'].", ".$_SESSION['ID_USUARIO'].", '".$fecha_actual->format("Y-m-d")."', '".$hora_actual."','Cambio de CONSIGNATARIO','".$pc_nom_ip."');");
						}
						else
						{
							...
							`ope_detalle` = 'Cambio de CONSIGNATARIO',
							`nom_pc_ip` = '".$pc_nom_ip."'
							WHERE `id_movimiento` = ".$id_movimiento."
...
<?php
	...
	else
	{
		...
		*/
	  	echo '<p>La operaci&oacute;n se registr&oacute; <span>Satisfactoriamente</span>.</p>';
	  	/*<p class="details">| Posted by <a href="#">SiteAdmin </a> | Categories: <a href="#">General</a> | Comments: <a href="#">73</a> |</p>*/
		// BOTONES PARA REGRESAR A LA VENTENA ANTERIOR
?>
		<p style="text-align:center;"><input class="button" type="button" name="btn_regresar" id="btn_regresar" value="Modificar otro Giro" style="width:150px;" onclick="this.disabled = 'true'; this.value = 'Enviando...';location.href='g_cambiar_consig.php';" ></p>
<?php
		echo '<!-- Limpiar Unidad del Contenido -->';
		echo '<hr class="clear-contentunit" />';
	}
